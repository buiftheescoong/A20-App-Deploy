...
export default function ComplianceReport({ isPassed, errors, suggestions }: ComplianceReportProps) {
            ...
            return (
              <div
                ...
                className={`flex items-start gap-3 p-3 rounded-lg border ${
                  hasError ? 'bg-red-50 border-red-100' : 'bg-green-50 border-green-100'
                }`}
              >
                <span className="text-lg">{hasError ? '❌' : '✅'}</span>
                <span className={`text-sm ${hasError ? 'text-red-700' : 'text-green-700'}`}>
                  {item}
                </span>
              </div>
            );
              ...
              {errors.map((err, i) => (
                <div key={i} className="p-4 rounded-xl bg-red-50 border border-red-100">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-sm text-red-700 font-medium">{err.issue}</p>
                      {err.suggestion && (
